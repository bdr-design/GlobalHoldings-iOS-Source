(()=>{
  ...
  async function plan({assets,routes,originFor,usable,provider=globalThis.GH_MAP_PROVIDER,seed=1,routeCount=routes.length,routeCapacity=()=>DEFAULT_ROUTE_CAPACITY,targetRouteLoad=null,initialLoad=()=>0,signal,onProgress=()=>{},intervalMs=1100,yieldEvery=DEFAULT_PLAN_YIELD_EVERY,yieldControl=yieldToUI}){
    ...
    for(const [index,asset]of ordered.entries()){
      if(index>0&&index%yieldEvery===0){if(signal?.aborted)throw new Error('أُلغي حساب المسارات');await yieldControl();}
  ...
  const API=Object.freeze({VERSION,BATCH_SIZE,MAX_ATTEMPTS,DEFAULT_ROUTE_CAPACITY,DEFAULT_PLAN_YIELD_EVERY,plan,pointFor,makeRoute});globalThis.GH_ROAD_PLANNER=API;if(globalThis.window&&window!==globalThis)window.GH_ROAD_PLANNER=API;
})();
