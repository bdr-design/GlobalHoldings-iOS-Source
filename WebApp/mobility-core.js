(()=>{
  ...
  function ensure(state){
    ...
    if(m.routeSchemaVersion!==3){
      ...
      for(const trip of m.activeTrips){trip.centerId=trip.centerId||'RUH';const zones=zonesFor(state,trip.centerId),from=zones.find(z=>z.id===trip.fromZone)||zones[0],key=streetRouteKey(trip.centerId,trip.fromZone,trip.toZone),cached=m.streetRoutes[key],verified=validStreetRoute(cached?.route);trip.route=verified?clone(cached.route):[[...from.coords],[...from.coords]];trip.routeLocked=true;trip.routeVersion=3;trip.routeVerified=verified;trip.progress=verified?Math.max(0,Math.min(1,Number(trip.progress)||0)):0;trip.dueAt=verified&&Number.isFinite(Number(trip.dueAt))?Number(trip.dueAt):null;trip.routeSource=verified?'OSRM · شبكة الشوارع الفعلية':'بانتظار مسار شارع موثّق';if(!verified&&cached)delete m.streetRoutes[key];}
  ...
  const API={VERSION,ZONES,CLASSES,CAPITALS,ensure,launch,buyFleet,onSimulationTime,snapshot,centerSnapshot,centerClusters,liveVehicles,vehiclePosition,pendingStreetRoutes,cacheStreetRoute,routePosition,urbanPath,zonesFor,capitalMeta,centerMeta,findVehicle,serviceVehicle,sellVehicle,render,renderFleet,execute};globalThis.GH_MOBILITY_CORE=API;globalThis.GH_DOMAIN_COMMANDS?.register?.('mobility',API);if(globalThis.window&&window!==globalThis)window.GH_MOBILITY_CORE=API;if(typeof module!=='undefined'&&module.exports)module.exports=API;
})();
