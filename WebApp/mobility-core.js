(()=>{
  ...
  function dispatch(m,at,centerId,zones){
    ...
    for(const request of queued){
      ...
      const from=zones.find(z=>z.id===request.fromZone)||zones[0],to=zones.find(z=>z.id===request.toZone)||from,key=streetRouteKey(centerId,request.fromZone,request.toZone),cached=m.streetRoutes[key],verified=validStreetRoute(cached?.route),route=verified?clone(cached.route):[[...from.coords],[...from.coords]],pickup=45+Math.round(random(`pickup:${request.id}`,25,150)),roadSeconds=Math.max(180,Number(cached?.durationSeconds)||Math.round(request.distanceKm/30*3600)),duration=pickup+roadSeconds;request.status='active';request.acceptedAt=at;request.vehicleId=vehicle.id;request.driverId=driver.id;request.tripId=id(m,'TRIP');vehicle.status='moving';vehicle.tripId=request.tripId;vehicle.routeLocked=true;vehicle.routeVersion=3;driver.status='on-trip';driver.tripId=request.tripId;m.activeTrips.push({id:request.tripId,requestId:request.id,vehicleId:vehicle.id,driverId:driver.id,centerId,fromZone:request.fromZone,toZone:request.toZone,acceptedAt:at,startedAt:at,routingReadyAt:verified?at:null,routingDeadlineAt:verified?null:at+ROUTE_WAIT_TIMEOUT_SECONDS,dueAt:verified?at+duration:null,duration,pickupSeconds:pickup,distanceKm:Number(cached?.distanceKm)||request.distanceKm,fare:request.fare,progress:0,route,routeLocked:true,routeVersion:3,routeVerified:verified,routeSource:verified?'OSRM · شبكة الشوارع الفعلية':'بانتظار مسار شارع موثّق'});m.kpis.accepted++;assigned++;
    }
    return assigned;
  }
