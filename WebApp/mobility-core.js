(()=>{
  ...
  function ensure(state){
    ...
    m.schema='gh-mobility-v3';m.version=VERSION;
    ...
    if(m.routeSchemaVersion!==2){
      for(const [index,vehicle] of m.vehicles.entries()){const spec=CLASSES.find(x=>x.id===vehicle.assetClass);if(!spec)continue;const serial=Math.max(1,Number(String(vehicle.id||'').split('-').pop())||index+1);vehicle.manufacturer=vehicle.manufacturer||spec.manufacturer;vehicle.model=vehicle.model||spec.models[(serial-1)%spec.models.length];vehicle.realModel=vehicle.realModel||vehicle.model;vehicle.routeLocked=true;vehicle.routeVersion=2;vehicle.routeSource='GH Mobility · شبكة حضرية مقفلة';vehicle.centerId=vehicle.centerId||'RUH';vehicle.baseLocation=vehicle.baseLocation||(vehicle.centerId==='RUH'?'الرياض':centerMeta(state,vehicle.centerId)?.city)||'الرياض';}
      ...
      for(const trip of m.activeTrips){trip.centerId=trip.centerId||'RUH';const zones=zonesFor(state,trip.centerId),from=zones.find(z=>z.id===trip.fromZone)||zones[0],to=zones.find(z=>z.id===trip.toZone)||from;trip.route=urbanPath(from,to);trip.routeLocked=true;trip.routeVersion=2;trip.routeSource='GH Mobility · شبكة حضرية مقفلة';}
      m.routeSchemaVersion=2;
    }
  ...
  function centerMeta(state,capitalId){
    ...
    const facility=(state.customHubs||[]).find(f=>f.owned===true&&f.company==='mobility'&&f.kind==='mobility-center'&&f.capitalId===capitalId);
  ...
  function vehiclePosition(state,vehicleId){const m=ensure(state),vehicle=m.vehicles.find(row=>row.id===vehicleId);if(!vehicle)return null;const zones=zonesFor(state,vehicle.centerId),trip=m.activeTrips.find(row=>row.vehicleId===vehicleId);if(trip){const from=zones.find(zone=>zone.id===trip.fromZone)||zones[0],to=zones.find(zone=>zone.id===trip.toZone)||from;return routePosition(urbanPath(from,to),trip.progress);}return zones.find(zone=>zone.id===vehicle.zoneId)?.coords||centerMeta(state,vehicle.centerId)?.coords||null;}
  function findVehicle(state,vehicleId){return ensure(state).vehicles.find(vehicle=>vehicle.id===vehicleId)||null;}
  function serviceVehicle(ctx,{id:vehicleId}={}){const state=ctx.state||ctx,m=ensure(state),vehicle=findVehicle(state,vehicleId);if(!vehicle)throw new Error('mobility-vehicle-not-found');if(vehicle.status==='moving')throw new Error('mobility-vehicle-moving');if(!globalThis.GH_FINANCE_CORE?.execute)throw new Error('finance-core-missing');const transaction=globalThis.GH_TRANSACTION_CORE;if(!transaction?.execute)throw new Error('transaction-core-missing');const runner=transaction.isActive()?transaction.join:transaction.execute,out=runner(state,{label:'mobility-vehicle-service',apply:()=>{globalThis.GH_FINANCE_CORE.execute({state},'spend',{company:'mobility',amount:850,note:`صيانة ${vehicle.name}`,method:'تحويل صيانة',line:'maintenance',counterparty:'GH Mobility Service Network'});vehicle.condition=100;vehicle.battery=100;vehicle.lastServiceAt=now(state);m.events.unshift({id:id(m,'MOB-EV'),at:now(state),type:'VEHICLE_SERVICED',vehicleId});return vehicle;}});return out.value;}
  function sellVehicle(ctx,{id:vehicleId}={}){const state=ctx.state||ctx,m=ensure(state),vehicle=findVehicle(state,vehicleId);if(!vehicle)throw new Error('mobility-vehicle-not-found');if(vehicle.status==='moving')throw new Error('mobility-vehicle-moving');if(!globalThis.GH_FINANCE_CORE?.execute)throw new Error('finance-core-missing');const hr=globalThis.GH_HR_CORE?.ensure?.(state);if(!hr)throw new Error('hr-core-missing');const transaction=globalThis.GH_TRANSACTION_CORE;if(!transaction?.execute)throw new Error('transaction-core-missing');const runner=transaction.isActive()?transaction.join:transaction.execute,out=runner(state,{label:'mobility-vehicle-sale',apply:()=>{const proceeds=Math.round((Number(vehicle.purchasePrice)||0)*.6);globalThis.GH_FINANCE_CORE.execute({state},'credit',{company:'mobility',amount:proceeds,note:`بيع ${vehicle.name}`,method:'تحويل مشتري',taxable:false,counterparty:'سوق المركبات المعتمدة'});m.vehicles=m.vehicles.filter(row=>row.id!==vehicleId);const driver=m.drivers.find(row=>row.id===vehicle.driverId||row.assetId===vehicleId);if(driver)m.drivers=m.drivers.filter(row=>row.id!==driver.id);const contracts=hr.employmentContracts;const exact=contracts.find(row=>row.assetId===vehicleId&&row.automaticAssetStaffing&&row.status==='ساري'),legacy=contracts.find(row=>row.company==='mobility'&&row.automaticAssetStaffing&&row.status==='ساري'&&String(row.center||'').includes(vehicle.baseLocation)),contract=exact||legacy;if(contract){contract.count=Math.max(0,(Number(contract.count)||0)-1);contract.salary=Math.max(0,(Number(contract.salary)||0)-6000);if(!contract.count){contract.status='منتهي';contract.endedDay=Math.floor(now(state)/86400);}}m.status=m.vehicles.length?'active':'not-launched';m.events.unshift({id:id(m,'MOB-EV'),at:now(state),type:'VEHICLE_SOLD',vehicleId,proceeds});return {vehicle,proceeds};}});return out.value;}
  ...
  function execute(ctx,cmd,p={}){if(cmd==='buy-fleet')return buyFleet(ctx,p);if(cmd==='service-vehicle')return serviceVehicle(ctx,p);if(cmd==='sell-vehicle')return sellVehicle(ctx,p);if(cmd==='launch')return launch(ctx);throw new Error(`Unknown Mobility command: ${cmd}`);}
  const API={VERSION,ZONES,CLASSES,CAPITALS,ensure,launch,buyFleet,onSimulationTime,snapshot,centerSnapshot,centerClusters,liveVehicles,vehiclePosition,routePosition,urbanPath,zonesFor,capitalMeta,centerMeta,findVehicle,serviceVehicle,sellVehicle,render,renderFleet,execute};globalThis.GH_MOBILITY_CORE=API;globalThis.GH_DOMAIN_COMMANDS?.register?.('mobility',API);if(globalThis.window&&window!==globalThis)window.GH_MOBILITY_CORE=API;if(typeof module!=='undefined'&&module.exports)module.exports=API;
})();
