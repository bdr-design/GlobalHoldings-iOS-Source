(() => {
  ...
  function transferPurposeMeta(x,sandboxCover=false){
    if(sandboxCover)return {category:'تغطية وضع اللعب',detail:'تغطية داخلية بسبب تفعيل الأموال غير المحدودة؛ ليست دفعة من طرف خارجي.',method:'قيد تمويل داخلي',documents:'—',refs:x.reference||x.id||'—'};
  ...
  function transferArt(x){
    const company=documentCompanyKey(x),i=financeDocumentIdentity(company),ref=x.reference||x.id||`TR-${Math.floor(x.at||0)}-${String(Math.round(x.amount||0)).slice(-7)}`,
      ...
      footerNote=sandboxCover?'قيد داخلي لوضع اللعب · ليس حوالة مصرفية خارجية':'سجل مالي موثق ومربوط بسبب ومستندات العملية',
