(() => {
  ...
  window.GH_BUSINESS_WORLD.execute({state},'sync-world',{
    ...
    opportunities:contracts.map(x=>({...x,status:state.acceptedContracts?.includes(x.id)?'نشط':state.contractRegistry?.[x.id]?.status==='بانتظار التوقيع'?'بانتظار التوقيع':state.failedBids?.includes(x.id)?'خسر العرض':'متاحة'}))
  });
