(() => {
  ...
  const initialAdvanced = () => ({
    schema:2,
    ai:{mode:'advisory',approvalLimit:0,automationDisabled:true,lastOptimization:0,recommendations:[],routeAdvice:[],baseAdvice:[],requests:[],delegations:[],requestArchive:[],executionLog:[],monitoringLetters:[],annualPlans:{},messages:[
  ...
  const migrate = state => {
    ...
    if(ai.restrictedModeVersion!==VERSION){
      const legacy=[...(ai.requests||[]),...(ai.baseAdvice||[]),...(ai.recommendations||[]),...(ai.routeAdvice||[])].filter(Boolean);
    ...
    ai.baseAdvice=[];ai.recommendations=[];ai.routeAdvice=[];ai.requests=[];ai.mode='advisory';ai.approvalLimit=0;ai.automationDisabled=true;
    for(const plan of Object.values(ai.annualPlans||{}))if(plan&&plan.status==='approved'){plan.status='paused';plan.pausedReason='AI advisory-only';}
  ...
  function proactiveReview(){return 0;}
