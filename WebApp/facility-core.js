...
function execute(ctx,cmd,p={}){const s=ctx.state||ctx;ensure(s);const f=p.id?find(s,p.id):null;verifyManagementOwner(f,cmd,p);const m=f?model(s,f):null;
 ...
 if(cmd==='expand'){const cost=num(p.cost||12000000),payment=purchase(s,ownerCompany(f),cost,`توسعة ${f.name}`,p.contractor||`مقاول توسعة ${f.name}`);m.level++;if(p.addCapacity!=null)m.capacity=Math.max(m.capacity,Number(p.currentCapacity)||0)+Math.max(1,Number(p.addCapacity)||1);else m.capacity=Math.round(m.capacity*Math.max(1.01,Number(p.factor)||1.25));m.budget+=num(p.budgetAdd||3000000);value(s,num(p.groupValueAdd||cost*.7));return {facilityId:f.id,capacity:m.capacity,paymentRef:payment?.cheque?.id||null,invoiceRef:payment?.invoice?.number||null,message:log(s,m,'اكتملت توسعة القدرة التشغيلية.')};}
 if(cmd==='hire'){
...
const API={VERSION,DEFAULT_ASSET_CAPACITY,ensure,all,find,model,ownerCompany,assetCapacity,assetOccupancy,availableAssetCapacity,verifyDirectorySite,verifyGlobalSite,validate,execute};globalThis.GH_FACILITY_CORE=API;globalThis.GH_DOMAIN_COMMANDS?.register?.('facilities',API);if(globalThis.window&&window!==globalThis)window.GH_FACILITY_CORE=API;if(typeof module!=='undefined'&&module.exports)module.exports=API;
})();
