...
(async()=>{
  ...
  const companyDirectory=async company=>{await panel('companies');await page.click('[data-companytab="subs"]');await page.click(`[data-open="companyManage"][data-arg="${company}"]`);await page.click(`[data-open="companyFacilities"][data-arg="${company}"]`);await page.click(`.open-facility-directory[data-kind="${company}"]`);assert.strictEqual(await page.inputValue('#worldKind'),company);};
