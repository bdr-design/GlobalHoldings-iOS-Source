...
// Every legacy Mobility trip is rebuilt onto the deterministic locked corridor.
...
const mobility=s.GH_MOBILITY_CORE.ensure(state),expected=s.GH_MOBILITY_CORE.urbanPath(s.GH_MOBILITY_CORE.ZONES.find(z=>z.id==='KAFD'),s.GH_MOBILITY_CORE.ZONES.find(z=>z.id==='AIRPORT'));
assert.strictEqual(mobility.activeTrips[0].routeLocked,true);assert.strictEqual(mobility.activeTrips[0].routeVersion,2);assert.deepStrictEqual(mobility.activeTrips[0].route,expected);
...
const live=s.GH_MOBILITY_CORE.liveVehicles(state,10,{movingOnly:true});assert.strictEqual(live.length,1);assert.strictEqual(live[0].id,'MOB-V-302-A');assert.deepStrictEqual(live[0].route,expected);
...
scaleState.mobility={routeSchemaVersion:2,capitalCenters:[{id:'MOB-SCALE',facilityId:'MOB-SCALE',capitalId:'RUH',city:'الرياض',country:'السعودية',coords:[24.7136,46.6753]}],vehicles:[],drivers:[],rideRequests:[],activeTrips:[],tripArchive:[],events:[],sequence:0,kpis:{},lastDemandAtByCenter:{RUH:0}};
...
assert(css.includes('.vehicle-pin.mobility{width:22px;height:22px}'));assert(css.includes('width:auto!important;min-width:0!important')&&!css.includes('brand-copy{display:block!important;flex:1 1 auto!important;width:0!important'));assert(app.includes('const assetIndex=new Map((state.assets||[]).map'));assert(!app.includes('function suggestRoutesOnly'));
