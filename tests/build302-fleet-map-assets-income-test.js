...
assert(state.finance.cheques.some(row=>row.id===serviced.lastMaintenanceCheque&&row.status==='مصروف'));
...
const mobilityCashBeforeService=s.GH_FINANCE_CORE.operating(state,'mobility');const servicedCar=s.GH_DOMAIN_COMMANDS.dispatch({state},'mobility','service-vehicle',{id:'MOB-V-302-B'},{actor:'test'}).result;assert.strictEqual(servicedCar.condition,100);assert.strictEqual(servicedCar.battery,100);assert.strictEqual(s.GH_FINANCE_CORE.operating(state,'mobility'),mobilityCashBeforeService-850);assert(state.finance.cheques.some(row=>row.id===servicedCar.lastServiceCheque&&row.status==='مصروف'));
assert.throws(()=>s.GH_DOMAIN_COMMANDS.dispatch({state},'mobility','sell-vehicle',{id:'MOB-V-302-A'},{actor:'test'}),/moving/);
const mobilityCashBeforeTrip=s.GH_FINANCE_CORE.operating(state,'mobility'),verifiedDueAt=mobility.activeTrips.find(row=>row.id==='TRIP-302').dueAt;mobility.lastDemandAtByCenter.RUH=verifiedDueAt+1;s.GH_MOBILITY_CORE.onSimulationTime({state},verifiedDueAt+1);const mobilityTripProfit=100-(100*.10+100*.16);
// BUILD306: completed-trip cash is durable immediately, but reaches the current
// account only at the atomic end-of-day boundary shared by every company.
assert.strictEqual(s.GH_FINANCE_CORE.operating(state,'mobility'),mobilityCashBeforeTrip);assert(Math.abs(state.finance.pendingDailyCash.mobility-mobilityTripProfit)<1e-9);assert(s.GH_FINANCE_CORE.performance(state,'mobility').currentNet>0);assert(!state.finance.invoices.some(row=>row.company==='mobility'&&row.simulation===true));
const mobilityAccruals=s.GH_FINANCE_CORE.execute({state},'consume-trip-accruals',{});s.GH_FINANCE_CORE.execute({state},'settle-daily-cash',{company:'mobility',amount:mobilityAccruals.cash.mobility,day:Math.floor(state.simSeconds/86400),reference:'DAY-CASH-MOBILITY-BUILD302'});assert(Math.abs(s.GH_FINANCE_CORE.operating(state,'mobility')-(mobilityCashBeforeTrip+mobilityTripProfit))<1e-9);assert(state.finance.transfers.some(row=>row.reference==='DAY-CASH-MOBILITY-BUILD302'&&row.status==='منفذة'));
...
// Large dispatch batches assign each vehicle/driver once without repeated
// full-fleet searches, while each active street pair remains exclusive.
...
const scaleZones=s.GH_MOBILITY_CORE.zonesFor(scaleState,'RUH'),scalePairs=[];for(const from of scaleZones)for(const to of scaleZones)if(from.id!==to.id)scalePairs.push([from.id,to.id]);
for(let i=0;i<500;i++){const [fromZone,toZone]=scalePairs[i%scalePairs.length];scaleState.mobility.rideRequests.push({id:`SCALE-R-${i}`,status:'queued',centerId:'RUH',fromZone,toZone,distanceKm:30,service:i%2?'eco-ev':'comfort',fare:80});}
s.GH_MOBILITY_CORE.onSimulationTime({state:scaleState},0);
assert.strictEqual(scaleState.mobility.activeTrips.length,scalePairs.length);assert.strictEqual(new Set(scaleState.mobility.activeTrips.map(row=>row.exclusiveRouteKey)).size,scalePairs.length);assert.strictEqual(new Set(scaleState.mobility.activeTrips.map(row=>row.vehicleId)).size,scalePairs.length);assert.strictEqual(new Set(scaleState.mobility.activeTrips.map(row=>row.driverId)).size,scalePairs.length);assert(scaleState.mobility.activeTrips.every(row=>scaleState.mobility.vehicles.find(vehicle=>vehicle.id===row.vehicleId)?.driverId===row.driverId),'large dispatch broke the fixed vehicle-driver pairing');
...
assert(app.includes('minRealSliceSeconds:.5'));assert(app.includes('allowedSpeeds:[0,30,60,120,300,600]'));assert(app.includes('snapshot:simulationAssetSnapshot(asset)'));assert(!app.includes('profile:state.profile,crew:state.crew,advanced:state.advanced,realism:state.realism'));
