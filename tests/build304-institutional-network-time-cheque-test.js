...
assert(app.includes("runDurableStateCommand('create-road-route'")&&app.includes("'routes','create-with-cache'"),'road geometry and cache must commit as one durable command');
assert(!app.includes('cancelRoadRoutePlacement')&&!app.includes('cancelMapPlacement')&&!app.includes('changeMapPlacement')&&!app.includes('confirmMapPlacement'),'free map placement must not survive beside the two-centre route owner');
