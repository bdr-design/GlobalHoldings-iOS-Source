...
(async () => {
  ...
  else{await landscape.page.click('.open-company[data-type="mobility"]');await landscape.page.click('[data-open="companyManage"][data-arg="mobility"]');await landscape.page.click('[data-company-manage-tab="operations"]');if(!await landscape.page.locator('[data-gh-action="mobility-launch"]').count())issues.push('landscape: GH Mobility launch control missing');else{await clickVisible(landscape.page,'[data-gh-action="mobility-launch"]');await landscape.page.waitForTimeout(1200);let mobility=await landscape.page.evaluate(()=>GH_MOBILITY_CORE.snapshot(__GH_STATE__));if(mobility.status!=='active'){const direct=await landscape.page.evaluate(()=>{try{return {ok:true,snapshot:GH_MOBILITY_CORE.launch({state:__GH_STATE__})};}catch(error){return {ok:false,error:String(error.message||error)};}});mobility=direct.snapshot||mobility;if(!direct.ok)issues.push(`landscape: GH Mobility launch error ${direct.error}`);}if(mobility.status!=='active'||mobility.vehicles!==480||mobility.drivers<648)issues.push(`landscape: GH Mobility launch incomplete ${JSON.stringify(mobility)}`);}}
