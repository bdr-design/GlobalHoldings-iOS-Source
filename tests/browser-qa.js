...
(async () => {
  ...
  async function openAt(name, viewport) {
    ...
    const founderGeometry=await page.evaluate(()=>{const flow=document.querySelector('#founderFlow'),sheet=document.querySelector('.incorporation-form'),controls=[...flow.querySelectorAll('input:not([type="file"]),select,#founderReview')].filter(node=>node.getClientRects().length);return{flow:{clientWidth:flow.clientWidth,scrollWidth:flow.scrollWidth},sheet:{clientWidth:sheet.clientWidth,scrollWidth:sheet.scrollWidth},controls:controls.map(node=>node.getBoundingClientRect().height),viewport:window.innerWidth};});
    if(founderGeometry.flow.scrollWidth>founderGeometry.flow.clientWidth+1||founderGeometry.sheet.scrollWidth>founderGeometry.sheet.clientWidth+1||founderGeometry.controls.some(height=>height<44))issues.push(`${name}: founder contract geometry is unsafe ${JSON.stringify(founderGeometry)}`);
    await page.selectOption('#founderMode', 'sandbox');
    await page.click('#founderReview');await page.locator('#founderForm button[type="submit"]').click();await page.waitForFunction(()=>__GH_STATE__.onboardingComplete);
  ...
  if(!await landscape.page.locator('.open-company[data-type="mobility"]').count())issues.push('landscape: GH Mobility company card missing');
    ...
    if(!opened)issues.push('landscape: GH Mobility company did not open through its visible UI control');
      ...
      await clickVisible(landscape.page,'.open-facility-directory[data-kind="mobility"]');await landscape.page.selectOption('#worldCountry','SA');await clickVisible(landscape.page,'.open-directory-site[data-key="site:mobility:RUH"]');
  ...
  const mobilityEvidence=await landscape.page.evaluate(()=>{const marker=document.querySelector('.asset-marker.mobility'),sprite=marker?.querySelector('.vehicle-sprite'),image=sprite?.querySelector('img'),markerRect=marker?.getBoundingClientRect(),spriteStyle=sprite?getComputedStyle(sprite):null;return{snapshot:GH_MOBILITY_CORE.snapshot(__GH_STATE__),live:GH_MOBILITY_CORE.liveVehicles(__GH_STATE__).filter(x=>x.phase==='moving').length,mapMarkers:document.querySelectorAll('.asset-marker.mobility').length,markerSize:markerRect?{width:markerRect.width,height:markerRect.height}:null,sprite:spriteStyle?{src:image?.getAttribute('src')||'',loaded:!!(image?.complete&&image.naturalWidth>0),width:parseFloat(spriteStyle.width),height:parseFloat(spriteStyle.height),transform:spriteStyle.transform}:null};});
  ...
  if(mobilityEvidence.mapMarkers&&(!mobilityEvidence.markerSize||mobilityEvidence.markerSize.width<44||mobilityEvidence.markerSize.height<44||!mobilityEvidence.sprite||mobilityEvidence.sprite.src!=='assets/images/map-mobility-sedan-topdown.webp'||!mobilityEvidence.sprite.loaded||mobilityEvidence.sprite.width!==12||mobilityEvidence.sprite.height!==24))issues.push(`landscape: GH Mobility sedan is not loaded, compact or touchably usable ${JSON.stringify(mobilityEvidence)}`);
