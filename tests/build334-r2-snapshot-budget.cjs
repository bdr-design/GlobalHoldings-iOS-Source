...
function fixture(count=1){
 ...
 Object.assign(e.s,{state:e.state,COMPANY_PLATFORM:e.s.GH_COMPANY_PLATFORM,SIMULATION_ASSET_ENGINE:require('../WebApp/simulation-asset-core.js'),simulationAssetRuntimeContext:()=>({workerCompatible:false}),routeTemplates:{'R2-ROUTE':route},competitorAssets:[],BASE_ROUTE_IDS:new Set(['R2-ROUTE']),clone(value){if(value&&typeof value==='object'&&'phase' in value&&'progress' in value)stats.assetCopies++;return value===undefined?undefined:JSON.parse(JSON.stringify(value));},routeDistance:()=>1000,queueAssetSaleFinalize:()=>{},processFinancialDay:()=>{},processMarket:()=>{},processAssetDraft:asset=>{asset.progress=.1;},diag:()=>{}});
...
test('budgeted snapshot keeps nested data immutable across a yielded chunk',()=>{const e=fixture();e.state.assets[0].specs={capacity:12};let observed;e.s.processAssetDraft=asset=>{observed=asset.specs.capacity;asset.specs.capacity=55;asset.progress=.8;};const job=e.job();assert.equal(job.runChunk(1),false);e.state.assets[0].specs.capacity=99;finishWork(job);assert.equal(observed,12);assert.equal(e.state.assets[0].specs.capacity,99);assert.equal(job.finish().committed,false);assert.equal(e.state.simSeconds,0);});
