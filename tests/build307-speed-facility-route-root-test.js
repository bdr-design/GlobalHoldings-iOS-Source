...
assert(motion.includes('maxPixelsPerSecond:72')&&motion.includes('distanceDuration=screenDistance/profile.maxPixelsPerSecond*1000'),'maximum simulation speed must be visually rate-limited independently of game time');
assert(app.includes('markerTweens.clear();markerTargetCadence.clear();'),'a rebuilt map must not retain stale visual cadence state');
