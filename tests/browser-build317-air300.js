...
(async()=>{
  ...
  try{
    ...
    // Regression for the uploaded incident: reopen the owned facility through
    // the canonical facilities register, then hire. HR helpers must remain in
    // command context, never inside the cloneable payload.
    await panel('control');await page.locator('[data-open="expansion"]').first().click();
    const manageFacility=page.locator(`[data-open="facilityManage"][data-arg="${base.id}"]`);
