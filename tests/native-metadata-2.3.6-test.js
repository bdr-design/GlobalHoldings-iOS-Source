...
assert(project.includes('MARKETING_VERSION: "3.0.0"'),'MARKETING_VERSION mismatch');
assert(project.includes('CURRENT_PROJECT_VERSION: "'+fs.readFileSync('BUILD','utf8').trim()+'"'),'CURRENT_PROJECT_VERSION must match BUILD');
assert(project.includes('CFBundleShortVersionString: "3.0.0"'),'Info.plist marketing version mismatch');
assert(project.includes('CFBundleVersion: "'+fs.readFileSync('BUILD','utf8').trim()+'"'),'Info.plist build mismatch');
assert(storage.includes('?? "3.0.0"'),'Native bundled version fallback mismatch');
assert(app.includes("const APP_VERSION = '3.0.0'"),'WebApp version mismatch');
