...
const p=sandbox.module.exports||sandbox.GH_PERSISTENCE;assert(p&&p.VERSION==='3.0.0');
const state={saveVersion:'2.0.0',simSeconds:172800,assets:[],market:[],advanced:{saveSlots:[null,null,null]}};
let out=p.saveSlot(0,state,{appVersion:'3.0.0'});assert(out.ok);assert.strictEqual(out.meta.day,3);assert(p.slotStatus(0).exists);
store.set('global-holdings-save-slot-2','{"saveVersion":"bad"}');assert(!p.slotStatus(1).exists);
out=p.loadSlot(0,{storageKey:'main',appVersion:'3.0.0'});assert(out.ok);assert(store.get('main').includes('"simSeconds":172800'));
