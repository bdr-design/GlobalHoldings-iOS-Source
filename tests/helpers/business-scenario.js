...
function scenario(){
 ...
 ctx.buyAsset=(type,tab,id,mode,qty,baseId,prepaid,_silent,requestRef)=>command('procurement','purchase-assets',{type,tab,item:s.GH_ASSET_CATALOG[type][tab].find(x=>x.id===id),base:state.globalBases.find(x=>x.id===baseId),supplier:ctx.supplierFor(),mode,qty,prepaid,upfront:item.price*qty,leadSeconds:60,requestRef}).orderId;
