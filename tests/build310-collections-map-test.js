...
test('assigned-only bulk departure remains distinct from all-truck allocation',()=>{
 assert(functions.get('departRouteAssets').includes('asset.type===type&&asset.routeId&&'));assert(app.includes('تشغيل المسارات المعيّنة فقط'));assert(app.includes('توزيع وتشغيل أسطول الشاحنات'));assert(app.includes('مسارات طريق مشتركة ذات سعة محددة'));
