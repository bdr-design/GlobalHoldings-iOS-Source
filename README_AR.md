...
BUILD305 يغلق عيوب النزاهة المكتشفة في تدقيق BUILD304: يفرض تطابق الشيك مع الشركة والقيمة والمستفيد والذمة، ويجعل تسلسل القيود ثابتًا بعد الأرشفة، ويستخدم إقفال البنك والطاقة لليوم نفسه. تمويل مشروع الطاقة يرسمل فائدة الإنشاء ولا يبدأ سداد الأصل قبل التشغيل التجاري، والتسهيلات البنكية لها جدول سداد حقيقي، وLCR/NSFR/CET1 تعتمد أصولًا والتزامات فعلية. رحلات Mobility غير المحسومة تحرر السيارة والسائق بعد 15 دقيقة، وكل السيارات المتحركة الزائدة عن سقف العرض تبقى ممثلة كمجموعة. حاكم الأداء يقيس إنشاء الشريحة وCommit الإقفال إضافة إلى runChunk. يبقى Save Schema `2.0.0` متوافقًا دون تغيير.

تثبيت BUILD305.1 قبل الدمج يجعل فائدة التسهيلات بين شركات المجموعة تسوية مالية ذرية واحدة: تحريك نقدي واحد، مستندا دخل/مصروف متقابلان، وP&L كامل بلا إعادة ترحيل للنقد عند الإقفال. كما يجعل قائمة سلامة المصدر مشتقة حتميًا من Git ومتحققة بالأداة نفسها محليًا وفي CI، ويمنع صور المحتوى البديلة من تلويث مربعات Leaflet عند انقطاع الشبكة، ويعرض حالة خريطة غير متصلة صريحة. علامة Mobility تملك هدف لمس `44×44` ونقطة مرئية `14×14`. لا يغير هذا التثبيت الإصدار أو رقم البناء أو Save Schema.
