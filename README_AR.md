...
BUILD309 يوحّد سعة المنشأة بين الشراء والتسليم والأسطول في `Facility Core` واحد، ويرفع الشراء اليدوي الذري إلى `1000` أصل مع توزيع الكمية على منشآت الشركة المتوافقة. كما يثبت فتح المراكز والقواعد خارج الرياض، ويجعل الشيكات والحوالات والتسوية اليومية تغيّر الحساب الجاري الفعلي بقيد متوازن ومرجع قابل لإعادة المحاولة دون خصم مزدوج.

مركز المسارات هو الواجهة الوحيدة للتعيين والتشغيل. المسار البري لا ينشأ إلا بين مركزين لوجستيين مملوكين وبعد نجاح هندسة الطريق الفعلية. زر «مغادرة جماعية لمسارات مختلفة» ينفذ أمرًا ذريًا خاصًا بقطاع واحد: يولد لكل طائرة أو سفينة ممرًا دوليًا مستقلاً، أو يوزع الشاحنات على مسارات برية مستقلة موجودة؛ أي نقص أو تعارض يلغي الدفعة كلها بلا حركة جزئية. حُذفت وحدة التشغيل الذاتي وواجهتها وصورها من Runtime، وتُنظّف مفاتيحها القديمة أثناء ترحيل الحفظ.

حافظ البناء على عقد الطاقم الثابت الموجود: `12` لكل طائرة (`4` طيارين، `6` ضيافة، `2` مهندسين)، و`20` لكل سفينة (`2` قبطان، `14` بحارًا، `4` مهندسين)، و`3` لكل شاحنة (`2` سائقين، `1` فني). يُنشأ الطاقم والعقد والراتب تلقائيًا داخل معاملة شراء كل أصل، ويُنهى عقده عند خروج الأصل. كما عُزل فشل محاكاة الأصل داخل الأصل نفسه، وقُيد الإيقاف الشامل بأعطال سلامة الحفظ والزمن والمعاملة فقط.

أصبحت إعادة بناء طبقات الخريطة الثقيلة كل أربع ثوان، وتحديث المؤشرات كل نصف ثانية، مع سقف بصري `8px/s` عند `600×`. تظهر سيارات Mobility كسيدان بيضاء من الأعلى بحجم `12×24px` ومساحة لمس `44×44px`، وتدور وفق اتجاه الحركة المرئية. أثبت اختبار المتصفح شراء `200` شاحنة وتوزيعها `140 + 60` على مركزين خارج الرياض، وإنشاء `600` وظيفة طاقم، مع إطار p95 أقل من `80ms`.

راجع `docs/BUILD309_ROOT_STABILITY_AR.md` للعقود واختبارات القبول، و`docs/BUILD308_ROOT_SAFETY_NO_AUTONOMY_AR.md` للأساس السابق.
