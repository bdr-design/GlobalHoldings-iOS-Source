...
def main():
    ...
    manifest={'id':'gh-internal-2.9.1-build273-second-pass-dead-code-sweep','name':'Global Holdings 2.9.1 — Second-Pass Dead Code Sweep','version':VERSION,'minGameVersion':'2.9.0','packageType':'full-web','installMode':'clean-snapshot-v1','channel':'stable','signaturePayloadVersion':2,'createdAt':datetime.now(timezone.utc).isoformat(),'releaseNotes':'BUILD273: جولة تنظيف ثانية بدقة أعلى بعد توسيع أداة التدقيق الشاملة (BUILD271) لتغطي كل ملفات الأنوية الأخرى (14 ملفًا إضافيًا، كلها نظيفة فعليًا)، وإضافة فحص تكرار الثوابت وفحص النصوص ذاتية التناقض. حذفت 14 دالة إضافية مؤكدة الموت في app.js (منها 6 دوال واجهة تفويض ميزانية لم تُستخدم قط، ودالتان كانتا يتيمتين بالضبط بسبب حذف نظام المسارات التلقائية في BUILD270 نفسه ولم تُكتشفا وقتها)، و3 دوال في advanced-core.js (بقايا مؤكدة من نظام محفظة الأصول بالذكاء الاصطناعي المحذوف في BUILD260، نجت من ذلك التنظيف). أثناء الحذف، أخطأت بحذف دالة مساعدة حية (aiRoleLabel) عن طريق الخطأ لوقوعها بين دالتين ميتتين متجاورتين — التقطها فحص lint فورًا قبل أي رفع، وأُعيدت دون أي أثر. النتيجة النهائية مؤكدة باختبار صارم: صفر دالة غير مُستخدمة متبقية في app.js وadvanced-core.js. Save Schema يبقى 2.0.0.',
    ...
    for ext in ('saneiupdate','ghupdate'):
        (UPDATES/f'GlobalHoldings_Internal_Update_V291_BUILD273_SECOND_PASS_DEAD_CODE_SWEEP.{ext}').write_text(payload,encoding='utf-8')
