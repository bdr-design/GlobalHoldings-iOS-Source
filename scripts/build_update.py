...
def main():
    ...
    manifest={'id':'gh-internal-2.9.1-build276-full-game-panel-sweep','name':'Global Holdings 2.9.1 — Full-Game Panel Sweep','version':VERSION,'minGameVersion':'2.9.0','packageType':'full-web','installMode':'clean-snapshot-v1','channel':'stable','signaturePayloadVersion':2,'createdAt':datetime.now(timezone.utc).isoformat(),'releaseNotes':'BUILD276: مراجعة شاملة فعلية (لا افتراضية) لكل قسم في اللعبة - أداة تفتح 45 قسمًا حقيقيًا (39 لوحة رئيسية + 6 صفحات إدارة شركة لكل نوع) عبر نقرات فعلية حقيقية، وتفحص كل واحدة عن NaN ظاهرة، كلمة undefined حرفية، إخراج فارغ مريب، أو خطأ تشغيل فعلي. النتيجة: صفر مشاكل من هذا النوع عبر كل الأقسام. لوحتان فقط (واقعية الاقتصاد، شبكة الموانئ) كانتا بلا عنوان مخصص وتعرضان عنوانًا افتراضيًا عامًا (لوحة) - أُضيف لهما عنوان صحيح. حُوِّل هذا المسح لاختبار دائم يعمل عند كل بناء مستقبلي، فيصبح أي قسم جديد يُضاف بدون عنوان أو ينتج قيمة تالفة يوقف البناء تلقائيًا فورًا. Save Schema يبقى 2.0.0.',
    ...
    for ext in ('saneiupdate','ghupdate'):
        (UPDATES/f'GlobalHoldings_Internal_Update_V291_BUILD276_FULL_GAME_PANEL_SWEEP.{ext}').write_text(payload,encoding='utf-8')
