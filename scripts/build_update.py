...
def main():
    ...
    manifest={'id':'gh-internal-2.9.1-build270-dead-panel-cleanup-mobility-parity','name':'Global Holdings 2.9.1 — Dead Panel Cleanup & Mobility Map Parity','version':VERSION,'minGameVersion':'2.9.0','packageType':'full-web','installMode':'clean-snapshot-v1','channel':'stable','signaturePayloadVersion':2,'createdAt':datetime.now(timezone.utc).isoformat(),'releaseNotes':'BUILD270: مسح ممنهج لكل رابط data-open/data-panel في الواجهة مقابل كل مسار توجيه فعلي - صفر روابط معطلة، لكن كشف قسمين ميتين تمامًا (لا يشير إليهما أي زر إطلاقًا): لوحة منافسين قديمة (renderCompetitors) استبدلتها بالكامل لوحة الاستحواذ والدمج الحالية الأكثر اكتمالًا (فحص←عناية واجبة←عرض←سيطرة←تكامل)، واسم بديل قديم فارغ (renderMore) لقسم النظام الحالي. حذفتهما مع مدخلي التوجيه الخاصين بهما. حذف أيضًا createAutomaticRoutes/activateAutomaticRoute (8 كيلوبايت) - دالة كاملة غير مستدعاة من أي مكان لتعيين وتفعيل مسارات تلقائيًا دون تدخل اللاعب، تناقض مباشرة مبدأ الشراء والمسارات اليدوية بالكامل المعتمد في هذه اللعبة. تحقق أيضًا صريح (بطلب من اللاعب): Mobility تظهر فعليًا على الخريطة تمامًا مثل بقية الشركات - مراكزها كعلامات منشآت تحت فلتر الكل، ومركباتها كنقاط تحت فلتر GH Mobility المخصص - ولها تبويب أصول حقيقي (سجل الأسطول) ضمن نفس بنية لوحة الشركة المشتركة مع كل الشركات الأخرى، لا مسارًا منفصلًا أو ناقصًا. Save Schema يبقى 2.0.0.',
    ...
    for ext in ('saneiupdate','ghupdate'):
        (UPDATES/f'GlobalHoldings_Internal_Update_V291_BUILD270_DEAD_PANEL_CLEANUP_MOBILITY_PARITY.{ext}').write_text(payload,encoding='utf-8')
