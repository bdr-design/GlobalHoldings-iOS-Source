...
def main():
    ...
    manifest={'id':'gh-internal-2.9.1-build271-comprehensive-audit-tool','name':'Global Holdings 2.9.1 — Comprehensive Audit Tool & Orphaned Features','version':VERSION,'minGameVersion':'2.9.0','packageType':'full-web','installMode':'clean-snapshot-v1','channel':'stable','signaturePayloadVersion':2,'createdAt':datetime.now(timezone.utc).isoformat(),'releaseNotes':'BUILD271: أضاف scripts/comprehensive_audit.cjs - أداة تدقيق دائمة قابلة لإعادة التشغيل عند أي إصدار قادم، تجمع كل طرق الفحص المطوَّرة طوال هذه الجلسة (أوامر النطاق، معرفات الأزرار، مستمعات CSS، مسارات التنقل، الدوال غير المرجَّعة) في تقرير واحد. كشفت التشغيلة الأولى فئة كاملة من الميزات المنجزة خلفيًا بالكامل (منطق صحيح ومُختبر) لكن بلا أي زر حقيقي يستدعيها إطلاقًا: طرح عام أولي (IPO)، تحويل احتياطي الخزينة، كشف حساب خزينة، إصدار شيك سريع، رفع/خفض رواتب الطاقم، فتح قطاع، قروض بنكية، استثمار ESG - عُرضت جميعها للمراجعة دون تنفيذ تلقائي لأي منها. أُصلحت حالة واحدة فعليًا بثقة كاملة: زر مغادرة فردية حقيقي أُضيف لصفحة إدارة الأصل يستدعي departNow() الذي تحقق منه وأُصلح في BUILD258 نفسه ولم يكن له أي زر إطلاقًا - المسار الوحيد كان جماعيًا فقط. أُزيل أيضًا مسار إعادة تعيين مكرر ميت تمامًا (.reset-save) لصالح المسار الحقيقي الوحيد العامل (.new-game-direct) في لوحة الإعدادات. Save Schema يبقى 2.0.0.',
    ...
    for ext in ('saneiupdate','ghupdate'):
        (UPDATES/f'GlobalHoldings_Internal_Update_V291_BUILD271_COMPREHENSIVE_AUDIT_TOOL.{ext}').write_text(payload,encoding='utf-8')
