...
def main():
    ...
    manifest={'id':'gh-internal-2.9.1-build274-asset-requests-and-sections','name':'Global Holdings 2.9.1 — Asset Requests Unified, Sections Cleaned','version':VERSION,'minGameVersion':'2.9.0','packageType':'full-web','installMode':'clean-snapshot-v1','channel':'stable','signaturePayloadVersion':2,'createdAt':datetime.now(timezone.utc).isoformat(),'releaseNotes':'BUILD274: وحّد إحصاء طلبات الأصول في لوحة التشغيل الرئيسية — كانت خانتا \u0642\u064a\u062f \u0627\u0644\u0648\u0635\u0648\u0644 و\u062a\u0648\u0631\u064a\u062f\u0627\u062a \u062c\u0627\u0631\u064a\u0629 تحسبان نفس التعبير حرفيًا (نفس المصفوفة ونفس الفلتر) فتعرضان دائمًا نفس الرقم تحت تسميتين مختلفتين، وهذا التكرار هو بالضبط الالتباس في طلبات الأصول. استُبدل بإحصاء واحد واضح لعدد الطلبات المعلّقة مع قيمتها المالية الفعلية بدل رقم مكرر بلا فائدة إضافية. وجد أيضًا وأزال جدول تصنيف أقسام ثالثًا منفصلًا (panelRoot في app.js، غير الجدولين المُصلَحين سابقًا في BUILD270/272) كان لا يزال يحمل مفتاحي اللوحتين المحذوفتين؛ تحقق نهائي شامل عبر الملفين كليهما يؤكد الآن عدم بقاء أي أثر لهما في أي شكل. Save Schema يبقى 2.0.0.',
    ...
    for ext in ('saneiupdate','ghupdate'):
        (UPDATES/f'GlobalHoldings_Internal_Update_V291_BUILD274_ASSET_REQUESTS_AND_SECTIONS.{ext}').write_text(payload,encoding='utf-8')
