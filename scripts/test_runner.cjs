...
fs.mkdirSync(path.join(root,'.ci-output/ci'),{recursive:true});fs.writeFileSync(path.join(root,'.ci-output/ci/guard-results.json'),JSON.stringify({node:process.version,passed:rows.filter(r=>r.ok).length,total:rows.length,results:rows},null,2));
