...
final class GlobalGameStorage {
    ...
    private let attemptedBundleSnapshotKey = "GlobalHoldingsAttemptedBundleSnapshot"
    ...
    func applyWebBridgeUpdate(manifest: [String: Any], files: [[String: Any]], operationsJSON: String, preUpdateSaveGeneration: Int) throws -> AppliedUpdate {
        ...
        let parsed = try parse(manifest: manifest, operations: operations, operationsJSON: operationsJSON, rawFiles: files, rawDeletes: [])
    ...
    private func apply(_ update: ParsedUpdate, preUpdateSaveGeneration: Int) throws -> AppliedUpdate {
        ...
        return AppliedUpdate(version: update.version, build: update.build, manifest: update.manifest, operations: update.operations, operationsJSON: update.operationsJSON, replacedWebFiles: true)
    }
    ...
    private func prepareRuntimeForUpdate() throws -> Bool {
        ...
        if fileManager.fileExists(atPath: webURL.path) {
            ...
            // A verified source repair may retain Build334. Apply a NEW bundle
            // snapshot once, not on every launch or after an explicit rollback.
            let snapshot = Bundle.main.object(forInfoDictionaryKey: "GHSourceSnapshotSHA256") as? String
            let validSnapshot = snapshot.map { $0.count == 64 && $0.allSatisfy { "0123456789abcdef".contains($0) } } ?? false
            let snapshotUpgrade = sameSemantic && bundledBuild == installedBuild && validSnapshot
                && UserDefaults.standard.string(forKey: attemptedBundleSnapshotKey) != snapshot
            if semanticUpgrade || buildUpgrade || snapshotUpgrade {
                try installBundledBaseline(from: bundled, replacingVersion: recorded)
                if validSnapshot { UserDefaults.standard.set(snapshot, forKey: attemptedBundleSnapshotKey) }
            }
            return true
        }

        // A failed first-install copy must not leave a partial active runtime.
        try? fileManager.removeItem(at: stagingWebURL)
        do {
            try fileManager.copyItem(at: bundled, to: stagingWebURL)
            try validateStagedWebApp(at: stagingWebURL)
            try validateBundledCopy(source: bundled, staged: stagingWebURL)
            try fileManager.moveItem(at: stagingWebURL, to: webURL)
        } catch {
            try? fileManager.removeItem(at: stagingWebURL)
            throw error
        ...
        UserDefaults.standard.set(bundledBuild, forKey: installedBundledBuildKey)
        if let snapshot = Bundle.main.object(forInfoDictionaryKey: "GHSourceSnapshotSHA256") as? String {
            UserDefaults.standard.set(snapshot, forKey: attemptedBundleSnapshotKey)
        }
        return true
    }
    ...
    private func validateBundledCopy(source: URL, staged: URL) throws {
        func inventory(_ root: URL) throws -> [String: String] {
            guard let items = fileManager.enumerator(at: root, includingPropertiesForKeys: [.isRegularFileKey, .isSymbolicLinkKey]) else {
                throw UpdateError.message("تعذر فحص ملفات المصدر.")
            }
        ...
        guard try inventory(source) == inventory(staged) else {
            throw UpdateError.message("نسخة التشغيل المرحلية لا تطابق مصدر التطبيق.")
        }
    }
    ...
    private func parseUpdate(data: Data, requiresFormat: Bool) throws -> ParsedUpdate {
        ...
        return try parse(manifest: manifest, operations: operations, operationsJSON: operationsJSON, rawFiles: files, rawDeletes: deleted)
    }
